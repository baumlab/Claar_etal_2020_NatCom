...
p1 <- ggplot(aes(y = D.PaxC.log10, x = C.PaxC.log10, color=Year_Pre_Post), 
  ...
  scale_x_continuous(expand=c(0.01,0.01), 
                     ...
                     limits=c(min(metadata.SH.noFQ$C.PaxC.log10),
                              max(metadata.SH.noFQ$C.PaxC.log10,metadata.SH.noFQ$D.PaxC.log10)),
                     name="Clade C Abundance (log S:H)") +  
  scale_y_continuous(expand=c(0.01,0.01), 
                     breaks = c(-3,-2,-1.7,-1.398,-1.097,-0.796), 
                     labels = c(0.001,0.010,0.020,0.040,0.080,0.160),
                     limits=c(min(metadata.SH.noFQ$C.PaxC.log10),max(metadata.SH.noFQ$C.PaxC.log10,metadata.SH.noFQ$D.PaxC.log10)),
...
p4 <- p1 + geom_path(aes(group=coral_tag,color=Year_Pre_Post),arrow=arrow(length=unit(0.30,"cm"), type = "closed"), data = metadata.SH.noFQ.Ordered)
p4
...
pdf(file = "figures/Extended_Data/Figure_S7.pdf",width=16, height=8,useDingbats = FALSE)
# grid.arrange(p5,p5.2,p5.3,p5.4,nrow=2,ncol=2)
grid.arrange(p5,p5.2,p4,p5.3,p5.4,nrow=2,ncol=3, widths=c(1,1,2) , layout_matrix = rbind(c(1,2,3),c(4,5,3)))
dev.off()
