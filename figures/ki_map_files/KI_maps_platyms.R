...
symbols(villages$lon, villages$lat, circles=(villages$pop)/10, add=TRUE,inches=0.3, bg=alpha("red", 0.4))
## legend for village size
text(-157.4475, 1.663, "Village with 1500 people", cex=1.05)   
text(-157.4475, 1.6885, "Village with 1000 people", cex=1.05)   
text(-157.453, 1.714, "Village with 500 people", cex=1.05)   
#with(villages[!villages$village=="legend",], text(lon, lat, label=village, pos=1, offset=0.5, font=2, col="black"))
points(sites$lon, sites$lat, bg=alpha(sites$col,0.8), pch=21, cex=1.8)
#with(sites_platy, text(lon, lat, label=site, cex=0.7))
legend(-157.265, 2.08,legend=levels(sites$f.pressure), pt.bg=c("#8c510a","#d8b365","#c7eae5","#5ab4ac","#01665e"), pch=21, bty="n", pt.cex=1.8, cex=1.05)
text(-157.275, 2.01, "Human disturbance", srt=90, cex=1)
segments(-157.26, 1.957,-157.26, 2.056)
