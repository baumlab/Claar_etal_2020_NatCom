...
villages<-data.frame(c(1.989386333, 2.022090868, 1.983594483, 1.865048333, 1.665, 1.695, 1.717))
villages$lon<-c(-157.4760637, -157.4884092, -157.3683462, -157.5522183, -157.57, -157.57, -157.57)
villages$pop<-c(1879, 2311, 955, 441, 1500, 1000, 500)
villages$village<-c("London", "Tabwakea", "Banana", "Poland", "legend1500", "legend1000", "legend500")
colnames(villages)[1]<-"lat"
...
symbols(villages$lon, villages$lat, circles=(villages$pop)/10, add=TRUE,inches=0.3, bg=alpha("red", 0.4))
## legend for village size
text(-157.45, 1.663, "Village with 1500 people", cex=1.2)   
text(-157.45, 1.6925, "Village with 1000 people", cex=1.2)   
text(-157.4548, 1.715, "Village with 500 people", cex=1.2)   
#with(villages[!villages$village=="legend",], text(lon, lat, label=village, pos=1, offset=0.5, font=2, col="black"))
points(sites$lon, sites$lat, bg=alpha(sites$col,0.8), pch=21, cex=2)
#with(sites_platy, text(lon, lat, label=site, cex=0.7))
legend(-157.265, 2.08,legend=levels(sites$f.pressure), pt.bg=c("#8c510a","#d8b365","#c7eae5","#5ab4ac","#01665e"), pch=21, bty="n", pt.cex=2.0, cex=1.2)
text(-157.275, 2.01, "Human disturbance", srt=90, cex=1.1)
segments(-157.26, 1.96,-157.26, 2.058)
text(-157.3, 1.9, "Bay\nof\nWrecks")
text(-157.52, 1.82, "Vaskes\nBay")
#source("ki_map_files/KI_base_inset.R")
