...
# Specify order of levels in Year
levels(sample_data(phy97.f.c.coral)$Year) = c("2014", "2015Jan", "2015May","2015July", "2016March")
```

```{r, include=FALSE}
ps_alpha_div <- estimate_richness(phy97.f.c.coral, split = TRUE, measure = "Shannon")
ps_alpha_div$SampleID <- rownames(ps_alpha_div) %>%
  as.factor()
ps_alpha_div$CoralTag <- sample_data(phy97.f.c.coral)$CoralTag %>%
  as.factor()
ps_alpha_div$Year <- sample_data(phy97.f.c.coral)$Year %>%
  as.factor()

sample_data(phy97.f.c.coral)$SampleID <- rownames(sample_data(phy97.f.c.coral)) %>%
  as.factor()
sample_data(phy97.f.c.coral)$CoralTag <- sample_data(phy97.f.c.coral)$CoralTag %>%
  as.factor()
sample_data(phy97.f.c.coral)$Year <- sample_data(phy97.f.c.coral)$Year %>%
  as.factor()

ps_samp <- sample_data(phy97.f.c.coral) %>%
  unclass() %>%
  data.frame() %>%
