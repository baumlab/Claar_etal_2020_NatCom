...
#check sites in each field season
plyr::count(cn, c("Field.Season", "Site"))
tbl <- plyr::count(cn, c("Field.Season", "Site", "Species", "Cover"))
...
bleaching proportion by proportion of colonies at each disturbance level by species but just for platy and fpenta  

```{r data prep for ms figures, include=FALSE}

head(nov.tbl.no4)

#ok first lets add disturb level to the df
nov.pub <- nov.tbl.no4
nov.pub$disturb <- sitedata$pressure.group[match(nov.pub$site, sitedata$site)]
str(nov.pub)
nov.pub$disturb <- as.character(levels(nov.pub$disturb))[nov.pub$disturb]
...
#ok so it was originially going to be by the proportion but on 7 apr it changed to binary so just going to change that here
...
#old code for when it was by bleaching proportion and not binary

# #ok now seperate out platy and fpenta into their own df and summed to the disturbance level by bleaching proportion
# nov.pub.platy <- nov.pub[nov.pub$species.comb == "Platygyra sp.", ]
# nov.pub.platy <- aggregate(freq ~ disturb + bleach.prop.no4, nov.pub.platy, sum)
# head(nov.pub.platy)
# dim(nov.pub.platy)
# nov.pub.fpent <- nov.pub[nov.pub$species.comb == "F. pentagona", ]
# nov.pub.fpent <- aggregate(freq ~ disturb + bleach.prop.no4, nov.pub.fpent, sum)
# head(nov.pub.fpent)
# dim(nov.pub.fpent)
# 
# #so now we need to get proportion calculation
# 
# #first get a total of each species in each disturbance level
# platy.tot <- aggregate(freq~disturb, nov.pub.platy, sum)
# fpent.tot <- aggregate(freq~disturb, nov.pub.fpent, sum)
# 
# #ok add that total to the other df
# nov.pub.platy$disturb.tot <- platy.tot$freq[match(nov.pub.platy$disturb, platy.tot$disturb)]
# nov.pub.platy
# 
# nov.pub.fpent$disturb.tot <- fpent.tot$freq[match(nov.pub.fpent$disturb, fpent.tot$disturb)]
# nov.pub.fpent
# 
# #now calculate proportion
# nov.pub.platy$prop <- nov.pub.platy$freq/nov.pub.platy$disturb.tot
# nov.pub.platy
# 
# nov.pub.fpent$prop <- nov.pub.fpent$freq/nov.pub.fpent$disturb.tot
# nov.pub.fpent
# 
# #change structure and orders
# str(nov.pub.platy)
# nov.pub.platy$bleach.prop.no4 <- as.factor(nov.pub.platy$bleach.prop.no4)
# nov.pub.platy$disturb <- as.factor(nov.pub.platy$disturb)
# nov.pub.platy$disturb<-factor(nov.pub.platy$disturb, levels = c("Very high", "Medium", "Very low"))
# 
# str(nov.pub.fpent)
# nov.pub.fpent$bleach.prop.no4 <- as.factor(nov.pub.fpent$bleach.prop.no4)
# nov.pub.fpent$disturb <- as.factor(nov.pub.fpent$disturb)
# nov.pub.fpent$disturb<-factor(nov.pub.fpent$disturb, levels = c("Very high", "Medium", "Very low"))


```


```{r platy figure for ms, echo=FALSE}
...
```{r f penta figure for ms, echo=FALSE}
...
```{r combine the two figures, include=FALSE}

#pdf("figures/Extended_Data/FigureS_proprotion_disturb_2015d.pdf", width = 12)

grid.arrange(platy.nov, fpenta.nov, nrow=2, heights = c(0.45, 0.55))

grid.text("Proportion of colonies", x = unit(0.02, "npc"), y = unit(0.6, "npc"), gp = gpar(fontsize=15), rot = 90)
#grid.text("Platygyra ryukyuensis", x = unit(0.5, "npc"), y = unit(0.99, "npc"), gp = gpar(fontsize=15))
#grid.text("Favites pentagona", x = unit(0.5, "npc"), y = unit(0.545, "npc"), gp = gpar(fontsize=15))
...
##make a df of just the medium sites for the publication figure
head(cn.c)
unique(cn.c$site)
cn.c.pub <- cn.c[(cn.c$site == "8" | cn.c$site == "35"), ]
cn.c.pub <- droplevels(cn.c.pub)
unique(cn.c.pub$site)

#subset for  just platy and fpenta
unique(cn.c.pub$Species)
cn.c.pub <- cn.c.pub[(cn.c.pub$Species == "Platygyra.spp" | cn.c.pub$Species == "Favites.pentagona"), ]
cn.c.pub <- droplevels(cn.c.pub)
unique(cn.c.pub$Species)
head(cn.c.pub)

cn.sum.c.pub <-aggregate(Cover ~  Species + Field.Season + Bleached, cn.c.pub, sum)
head(cn.sum.c.pub)

#calculate the number of points at each site for each species (healthy and bleached) so we can get proportion of bleached vr healthy
cn.sum.c.species.pub <- aggregate(Cover ~ Species + Field.Season, cn.c.pub, sum)
head(cn.sum.c.species.pub)

#now add to the other dataframe 
cn.sum.c.pub$season.spec <- paste(cn.sum.c.pub$Field.Season, cn.sum.c.pub$Species, sep = ".")
cn.sum.c.species.pub$season.spec <- paste(cn.sum.c.species.pub$Field.Season, cn.sum.c.species.pub$Species, sep = ".")
cn.sum.c.pub$total.season.spec <- cn.sum.c.species.pub$Cover[match(cn.sum.c.pub$season.spec, cn.sum.c.species.pub$season.spec)]
head(cn.sum.c.pub)

#now calculate proportion
cn.sum.c.pub$prop <- cn.sum.c.pub$Cover/cn.sum.c.pub$total.season.spec
head(cn.sum.c.pub)

#changing bleaching to bleached
str(cn.sum.c.pub)
cn.sum.c.pub$Bleached <- as.character(levels(cn.sum.c.pub$Bleached))[cn.sum.c.pub$Bleached]
cn.sum.c.pub$Bleached <- ifelse(cn.sum.c.pub$Bleached == "Bleaching", "Bleached", cn.sum.c.pub$Bleached)
cn.sum.c.pub$Bleached <- as.factor(cn.sum.c.pub$Bleached)
levels(cn.sum.c.pub$Bleached)
...
```{r overall figure coralnet 4 sites publication, eval=FALSE, include=FALSE}

# cn.sum.c.pub <- cn.sum.c[(cn.sum.c$Species == "Platygyra.spp" | cn.sum.c$Species == "Favites.pentagona"), ]
# cn.sum.c.pub <- droplevels(cn.sum.c.pub)
# cn.sum.c.pub$Species <- as.character(levels(cn.sum.c.pub$Species))[cn.sum.c.pub$Species]
# cn.sum.c.pub$Species <- ifelse(cn.sum.c.pub$Species == "Platygyra.spp", "Platygyra ryukyuensis", cn.sum.c.pub$Species)
# cn.sum.c.pub$Species <- ifelse(cn.sum.c.pub$Species == "Favites.pentagona", "Favites pentagona", cn.sum.c.pub$Species)
# cn.sum.c.pub$Species <- as.factor(cn.sum.c.pub$Species)
# cn.sum.c.pub$Species<-factor(cn.sum.c.pub$Species, levels = c("Platygyra ryukyuensis", "Favites pentagona"))

#pdf("figures/Extended_Data/FigureS_proprtion_2015b-2016a.pdf", width = 12)

coralnet <- ggplot(cn.sum.c.pub, aes(x = Field.Season, y = prop, fill = Bleached)) +
  geom_bar(stat='identity', position='stack', color = "black") +
  ...
  theme(axis.text.x = element_text(angle = 0), plot.background=element_blank(), 
        ...
        axis.line = element_line(color = "black"), strip.text = element_blank() ) +
...
```{r figure coral net for ms platy, echo=FALSE}

cn.sum.c.pub.platy <- cn.sum.c.pub[cn.sum.c.pub$Species == "Platygyra.spp", ]

coralnet.platy <- ggplot(cn.sum.c.pub.platy, aes(x = Field.Season, y = prop, fill = Bleached)) +
  geom_bar(stat='identity', position='stack', color = "black") +
  labs(x = "", y = "", fill = "Status")+
    scale_fill_manual(values = bleachcols.pub) +
  scale_x_discrete(labels = c("KI2015b" = "May 2015", "KI2015c" = "July 2015", "KI2015d" = "November 2015", "KI2016a" = "March 2016")) +
...
```{r figure coral net for ms fpenta, echo=FALSE}

cn.sum.c.pub.fpent <- cn.sum.c.pub[cn.sum.c.pub$Species == "Favites.pentagona", ]

coralnet.fpent <- ggplot(cn.sum.c.pub.fpent, aes(x = Field.Season, y = prop, fill = Bleached)) +
  geom_bar(stat='identity', position='stack', color = "black") +
  labs(x = "", y = "", fill = "Status")+
    scale_fill_manual(values = bleachcols.pub) +
  scale_x_discrete(labels = c("KI2015b" = "May 2015", "KI2015c" = "July 2015", "KI2015d" = "November 2015", "KI2016a" = "March 2016")) +
...
###across the disturbance gradient but only for platy and fpenta  
  
```{r data prep for disturb, include=FALSE}

head(cn.sum.c.disturb)

#take out species not needed
cn.sum.c.disturb.pub <- cn.sum.c.disturb[(cn.sum.c.disturb$Species == "Platygyra.spp" | cn.sum.c.disturb$Species == "Favites.pentagona"), ]
cn.sum.c.disturb.pub <- droplevels(cn.sum.c.disturb.pub)
head(cn.sum.c.disturb.pub)
str(cn.sum.c.disturb.pub)
cn.sum.c.disturb.pub$Species <- as.character(levels(cn.sum.c.disturb.pub$Species))[cn.sum.c.disturb.pub$Species]
cn.sum.c.disturb.pub$Species <- ifelse(cn.sum.c.disturb.pub$Species == "Platygyra.spp", "Platygyra ryukyuensis", cn.sum.c.disturb.pub$Species)
cn.sum.c.disturb.pub$Species <- ifelse(cn.sum.c.disturb.pub$Species == "Favites.pentagona", "Favites pentagona", cn.sum.c.disturb.pub$Species)
cn.sum.c.disturb.pub$Species <- as.factor(cn.sum.c.disturb.pub$Species)
cn.sum.c.disturb.pub$Species<-factor(cn.sum.c.disturb.pub$Species, levels = c("Platygyra ryukyuensis", "Favites pentagona"))

str(cn.sum.c.disturb.pub)
cn.sum.c.disturb.pub$Fish.Level <- as.character(levels(cn.sum.c.disturb.pub$Fish.Level))[cn.sum.c.disturb.pub$Fish.Level]
cn.sum.c.disturb.pub$Fish.Level <- ifelse(cn.sum.c.disturb.pub$Fish.Level == "Very.High", "Very High", cn.sum.c.disturb.pub$Fish.Level)
cn.sum.c.disturb.pub$Fish.Level <- as.factor(cn.sum.c.disturb.pub$Fish.Level)
cn.sum.c.disturb.pub$Fish.Level<-factor(cn.sum.c.disturb.pub$Fish.Level, levels = c("Very High", "Medium"))
...
```{r prop by disturb platy with only 4 sites platy, echo=FALSE, warning=FALSE}

cn.sum.c.disturb.pub.platy <- cn.sum.c.disturb.pub[cn.sum.c.disturb.pub$Species == "Platygyra ryukyuensis", ]
cn.sum.c.disturb.pub.platy <- droplevels(cn.sum.c.disturb.pub.platy)

#pdf("figures/Extended_Data/FigureS_proprtion_2015b-2016a_Platy.pdf", width = 12)

platy <- ggplot(cn.sum.c.disturb.pub.platy, aes(x = Field.Season, y = prop, fill = Bleached)) +
  geom_bar(stat='identity', position='stack', color = "black") +
  facet_wrap(~Fish.Level, scales = "free_y") +
  labs(x = "", y = "Proportion of cover", fill = "Status")+
    scale_fill_manual(values = bleachcols.pub) +
  scale_x_discrete(labels = c("KI2015b" = "April/May\n2015", "KI2015c" = "July\n2015", "KI2015d" = "November\n2015", "KI2016a" = "March/April\n2016")) +
  theme(axis.text.x = element_text(angle = 0), plot.background=element_blank(), 
        panel.background = element_blank(), 
        strip.background = element_blank(), strip.text = element_text(size = 13),
        axis.line = element_line(color = "black"), legend.position="bottom", 
...
```{r prop by disturb platy with only 4 sites fpenta, echo=FALSE, warning=FALSE}

cn.sum.c.disturb.pub.fpenta <- cn.sum.c.disturb.pub[cn.sum.c.disturb.pub$Species == "Favites pentagona", ]
cn.sum.c.disturb.pub.fpenta <- droplevels(cn.sum.c.disturb.pub.fpenta)

#pdf("figures/Extended_Data/FigureS_proprtion_2015b-2016a_Fpenta.pdf", width = 12)

fpenta <- ggplot(cn.sum.c.disturb.pub.fpenta, aes(x = Field.Season, y = prop, fill = Bleached)) +
  geom_bar(stat='identity', position='stack', color = "black") +
  facet_wrap(~Fish.Level, scales = "free_y") +
  labs(x = "", y = "Proportion of cover", fill = "Status")+
    scale_fill_manual(values = bleachcols.pub) +
...
```{r combine the two , include=FALSE}

#pdf("figures/Extended_Data/FigureS_proprotion_disturb_2015b-2016a.pdf", width = 12)

grid.arrange(platy, fpenta, nrow=2, heights = c(0.45, 0.55))
...
```{r combine the 4 figures, include=FALSE}

pdf("figures/Extended_Data/FigureS5_coralnet_nov15.pdf", width = 12)
...
#add y axis labels
grid.text("Proportion of benthic community", x = unit(0.01, "npc"), y = unit(0.56, "npc"), gp = gpar(fontsize=15), rot = 90)
grid.text("Proportion of colonies", x = unit(0.51, "npc"), y = unit(0.55, "npc"), gp = gpar(fontsize=15), rot = 90)

#add panel lables
grid.text("a", x = unit(0.05, "npc"), y = unit(0.96, "npc"), gp = gpar(fontsize=18, face = "bold"))
grid.text("b", x = unit(0.55, "npc"), y = unit(0.96, "npc"), gp = gpar(fontsize=18, face = "bold"))
grid.text("c", x = unit(0.05, "npc"), y = unit(0.51, "npc"), gp = gpar(fontsize=18, face = "bold"))
grid.text("d", x = unit(0.55, "npc"), y = unit(0.51, "npc"), gp = gpar(fontsize=18, face = "bold"))

#add sample sizes
##panel a
grid.text("86", x = unit(0.12, "npc"), y = unit(0.935, "npc"), gp = gpar(fontsize=12))
grid.text("89", x = unit(0.225, "npc"), y = unit(0.935, "npc"), gp = gpar(fontsize=12))
grid.text("42", x = unit(0.329, "npc"), y = unit(0.935, "npc"), gp = gpar(fontsize=12))
grid.text("65", x = unit(0.432, "npc"), y = unit(0.935, "npc"), gp = gpar(fontsize=12))

##panel b
grid.text("19", x = unit(0.68, "npc"), y = unit(0.935, "npc"), gp = gpar(fontsize=12))
grid.text("88", x = unit(0.877, "npc"), y = unit(0.935, "npc"), gp = gpar(fontsize=12))
  
##panel c
grid.text("19", x = unit(0.12, "npc"), y = unit(0.495, "npc"), gp = gpar(fontsize=12))
grid.text("77", x = unit(0.225, "npc"), y = unit(0.495, "npc"), gp = gpar(fontsize=12))
grid.text("16", x = unit(0.329, "npc"), y = unit(0.495, "npc"), gp = gpar(fontsize=12))
grid.text("10", x = unit(0.432, "npc"), y = unit(0.495, "npc"), gp = gpar(fontsize=12))
  
##panel d
grid.text("4", x = unit(0.68, "npc"), y = unit(0.495, "npc"), gp = gpar(fontsize=12))
grid.text("31", x = unit(0.877, "npc"), y = unit(0.495, "npc"), gp = gpar(fontsize=12))
