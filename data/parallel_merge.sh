...
mkdir -p data/merge
iu-gen-configs data/rawdata/fastq/fastq_list.txt -o data/merge

# Merge paired reads for each sample (in parallel)
/home/ross/bin/parallel iu-merge-pairs {} --min-overlap-size 150 --enforce-Q30-check --marker-gene-stringent ::: data/merge/*.ini

# Filter sequences - keep only those with 3 mismatches or less (in parallel)
/home/ross/bin/parallel iu-filter-merged-reads {} --max-mismatches 3 ::: data/merge/*_MERGED

# Source QIIME on the Gates Lab server
